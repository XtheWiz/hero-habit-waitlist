...
import { BookOpen, Dumbbell, Shield, Sword, Zap, ChevronRight, Twitter, Globe, Trophy, Users, Heart, AlertCircle } from 'lucide-react';
...
const getEnvVar = (key) => {
  try {
    ...
    if (key === 'FIREBASE_CONFIG' && value) return JSON.parse(value);
...
const App = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const waitlistRef = isPreviewEnv 
        ? collection(db, 'artifacts', appId, 'public', 'data', 'waitlist')
        : collection(db, 'waitlist');
      
      // Rule 2: No complex queries. Fetch all and filter in JS memory for duplicate check.
      ...
      if (isDuplicate) {
        setAuthError("This email is already registered in our guild!");
        setIsLoading(false);
        return;
      }
      ...
      await addDoc(waitlistRef, {
        ...
        source: isPreviewEnv ? 'canvas_preview' : 'production'
      });
  ...
  return (
    <div className="min-h-screen font-sans bg-slate-950 text-slate-100 selection:bg-indigo-500 selection:text-white">
      <nav className="flex items-center justify-center px-6 py-8 mx-auto max-w-7xl">
        <div className="flex items-center gap-2 cursor-default group">
          <div className="p-2 transition-transform rounded-lg shadow-lg bg-gradient-to-br from-indigo-500 to-purple-600 shadow-indigo-500/20 group-hover:scale-110">
      ...
      <section className="px-6 pt-16 pb-32 mx-auto text-center max-w-7xl">
        ...
        <p className="max-w-2xl mx-auto mb-12 text-lg font-medium leading-relaxed text-slate-500 md:text-xl">
          ...
          Turn deep work into <span className="text-blue-400">Mana</span> and workouts into <span className="text-red-400">Might.</span>
        </p>
        ...
        <div className="relative max-w-md mx-auto">
          {!isSubmitted ? (
            <>
              <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row">
                ...
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="flex items-center justify-center gap-2 px-8 py-4 font-bold transition-all transform shadow-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 rounded-2xl shadow-indigo-600/20 active:scale-95 disabled:opacity-50"
                >
                  {isLoading ? "Joining..." : "Join Guild"} <ChevronRight className="w-5 h-5" />
                </button>
              </form>
              {authError && (
                <div className="flex items-center gap-2 p-3 mt-4 text-xs text-left text-red-400 border bg-red-500/10 border-red-500/20 rounded-xl animate-in fade-in slide-in-from-top-1">
            ...
            <div className="p-8 text-indigo-400 duration-500 border bg-indigo-500/10 border-indigo-500/20 rounded-2xl animate-in fade-in zoom-in">
              <p className="mb-2 text-xl italic font-black tracking-tight text-center uppercase">Welcome to the Guild, Hero!</p>
              <p className="text-sm font-medium leading-relaxed tracking-widest text-center uppercase opacity-80">Your spot is secured. Founder items incoming.</p>
            </div>
          )}
          <div className="mt-6 flex items-center justify-center gap-4 text-slate-600 text-[10px] uppercase font-bold tracking-[0.2em]">
            <span className="flex items-center gap-1"><Shield className="w-3 h-3" /> Cloud Secured</span>
            <span className="flex items-center gap-1"><Trophy className="w-3 h-3" /> Exclusive Loot</span>
            <span className="flex items-center gap-1"><Users className="w-3 h-3" /> 1.2k+ Waiting</span>
          </div>
        </div>
      </section>

      {/* Stats Preview Card */}
      <section id="stats" className="max-w-5xl px-6 mx-auto mb-32">
        <div className="bg-slate-900 border border-slate-800 p-10 rounded-[2.5rem] relative overflow-hidden shadow-2xl">
          ...
          <div className="grid items-center gap-16 md:grid-cols-2">
            <div>
              <h2 className="mb-8 text-4xl italic font-black tracking-tighter uppercase">Character Sheet</h2>
      ...
      <section id="dev" className="max-w-4xl px-6 py-32 mx-auto text-center">
        ...
        <p className="max-w-2xl mx-auto mb-10 text-lg italic font-medium leading-relaxed text-slate-400">
          ...
          I created this project to bridge the gap."
        </p>
      </section>

      <footer className="py-24 text-center border-t border-slate-900">
        <p className="text-slate-700 text-[10px] tracking-[0.5em] uppercase font-bold">Project Hero Habit • 2025</p>
      </footer>
    </div>
  );
};
